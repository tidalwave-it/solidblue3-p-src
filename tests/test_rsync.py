...
"""
    ...
    def test_final_stats_messages(self, rsync, presentation):
        ...
        assert ('notify_message', 'File list generation time: 2.870 seconds') in presentation.calls
        assert ('notify_message', 'File list transfer time: 0.000 seconds') in presentation.calls
    ...
    def test_final_stats_exact_sequence(self, rsync, presentation):
        feed(rsync, self.FINAL_STATS_INPUT)
...
class TestBuildingFileList:
    def test_building_file_list_triggers_indeterminate(self, rsync, presentation):
        feed(rsync, 'building file list ...')
        assert ('notify_progress_indeterminate',) in presentation.calls

    def test_building_file_list_triggers_status(self, rsync, presentation):
        feed(rsync, 'building file list ...')
...
class TestFileProgress:
    def test_partial_file_progress_updates_secondary_progress(self, rsync, presentation):
